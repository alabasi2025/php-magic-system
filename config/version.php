<?php
return [
    /*
    |--------------------------------------------------------------------------
    | Application Version
    |--------------------------------------------------------------------------
    |
    | This value is the version of your application. This value is used when
    | the framework needs to display the version number in views or logs.
    | 
    | ⚠️ IMPORTANT: This is the SINGLE SOURCE OF TRUTH for version number.
    | All other files should reference this config value.
    |
    */
    'number' => 'v2.9.1',
    /*
    |--------------------------------------------------------------------------
    | Version Name (Optional)
    |--------------------------------------------------------------------------
    |
    | A friendly name for this version (e.g., "Phoenix", "Titan", etc.)
    |
    */
    'name' => 'Vue.js Edition',
    /*
    |--------------------------------------------------------------------------
    | Release Date
    |--------------------------------------------------------------------------
    |
    | The date this version was released
    |
    */
    'release_date' => '2025-12-02',
    /*
    |--------------------------------------------------------------------------
    | Full Version String
    |--------------------------------------------------------------------------
    |
    | Complete version string including name
    |
    */
    'full' => 'SEMOP v2.9.1 - Vue.js Edition',
];
