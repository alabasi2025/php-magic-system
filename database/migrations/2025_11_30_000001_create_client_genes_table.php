...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('client_genes', function (Blueprint $table) {
            ...
            $table->string('client_code')->unique()->comment('كود العميل الفريد');
            $table->string('gene_name')->comment('اسم الجين');
            $table->boolean('is_active')->default(true)->comment('هل الجين مفعل؟');
            $table->json('configuration')->nullable()->comment('إعدادات الجين الخاصة بالعميل');
            ...
            $table->index(['client_code', 'gene_name']);
        });
    }
