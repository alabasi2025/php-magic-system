...
<div class="container-fluid" dir="rtl">
    ...
    <div class="stock-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="mt-3 mt-md-0">
                <a href="{{ route('inventory.stock-out.create') }}" class="btn btn-light btn-lg" style="border-radius: 12px; font-weight: 600; padding: 0.75rem 2rem;">
    ...
    <div class="main-card">
        ...
        <div class="card-body" style="padding: 2rem;">
            {{-- Search and Filters --}}
            <form action="{{ route('inventory.stock-out.index') }}" method="GET" class="mb-4">
                <div class="row">
                    ...
                    <tbody>
                        @forelse ($stockOuts as $stockIn)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('inventory.stock-out.show', $stockIn->id) }}" 
                                    ...
                                    @if($stockIn->status == 'pending')
                                        <form action="{{ route('inventory.stock-out.approve', $stockIn->id) }}" 
                                              method="POST" 
                                              class="d-inline">
                                            @csrf
                                            <button type="submit" 
                                        ...
                                        <form id="delete-form-{{ $stockIn->id }}" 
                                              action="{{ route('inventory.stock-out.destroy', $stockIn->id) }}" 
                                              method="POST" 
                                              class="d-none">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <h4>لا توجد أوامر صرف</h4>
                                        <p>ابدأ بإنشاء أول أمر صرف للنظام</p>
                                        <a href="{{ route('inventory.stock-out.create') }}" class="btn btn-gradient mt-3">
                                            <i class="fas fa-plus ml-2"></i>
                                            إنشاء أول أمر صرف
                                        </a>
                                    </div>
            ...
            @if($stockOuts->hasPages())
                <div class="d-flex justify-content-between align-items-center mt-4">
                    <div style="color: #64748b; font-weight: 600;">
                        عرض {{ $stockOuts->firstItem() ?? 0 }} إلى {{ $stockOuts->lastItem() ?? 0 }} من أصل {{ $stockOuts->total() }} أمر
                    </div>
                    <div>
