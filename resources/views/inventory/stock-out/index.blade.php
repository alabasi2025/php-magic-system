...
@section('title', 'أوامر الصرف المخزني')
...
<div class="inventory-page">
    <div class="container py-5">
        ...
        <div class="glass-header" data-aos="fade-down">
            <h1><i class="fas fa-box-open me-3"></i>أوامر الصرف المخزني</h1>
            <p class="subtitle">إدارة وتتبع جميع عمليات الصرف إلى المخازن</p>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card-3d" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="stat-label">إجمالي أوامر الصرف</div>
            </div>
        ...
        <div class="glass-table-container" data-aos="fade-up" data-aos-delay="400">
            <div class="table-header">
                <h2 class="table-title"><i class="fas fa-list me-2"></i>قائمة أوامر الصرف</h2>
                <a href="{{ route('inventory.stock-out.create') }}" class="btn-modern btn-primary-modern">
                    ...
                    <table class="modern-table">
                        ...
                        <tbody>
                            @foreach($stockOuts as $stockOut)
                                <tr>
                                    <td><strong>{{ $stockOut->movement_number }}</strong></td>
                                    <td>{{ $stockOut->warehouse->name ?? '-' }}</td>
                                    <td>{{ $stockOut->movement_date }}</td>
                                    <td>
                                        <span class="status-badge status-{{ $stockOut->status }}">
                                            @if($stockOut->status == 'pending') قيد الانتظار
                                            @elseif($stockOut->status == 'approved') معتمد
                                            @else مرفوض
                                            @endif
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('inventory.stock-out.show', $stockOut->id) }}" class="action-btn action-btn-view">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($stockOut->status == 'pending')
                                            <a href="{{ route('inventory.stock-out.edit', $stockOut->id) }}" class="action-btn action-btn-edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button class="action-btn action-btn-delete" onclick="confirmDelete({{ $stockOut->id }})">
