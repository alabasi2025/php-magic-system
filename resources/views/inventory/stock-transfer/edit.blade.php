...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-purple-50 to-pink-50 py-8 px-4 sm:px-6 lg:px-8" 
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-pink-500 via-rose-500 to-red-600 p-8 shadow-2xl">
            ...
            <div class="relative flex items-center justify-between">
                <div class="flex items-center space-x-reverse space-x-6">
                    ...
                    <div>
                        <h1 class="text-4xl font-bold text-white mb-2">تعديل أمر التحويل</h1>
                        <p class="text-pink-100">رقم الأمر: {{ $stockTransfer->movement_number }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl bg-red-50 border-r-4 border-red-500 p-6">
            <div class="flex items-center space-x-reverse space-x-4">
                ...
                <div>
                    ...
                    <p class="text-red-700">هذا الأمر {{ $stockTransfer->status === 'approved' ? 'معتمد' : 'مرفوض' }} ولا يمكن تعديله</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('inventory.stock-transfer.update', $stockTransfer->id) }}" method="POST" class="max-w-7xl mx-auto">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="date" name="movement_date" x-model="formData.movement_date" :disabled="isDisabled" required
                                   value="{{ $stockTransfer->movement_date }}"
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $stockTransfer->movement_number }}" readonly
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="notes" x-model="formData.notes" :disabled="isDisabled" rows="3"
                                      class="w-full rounded-xl border-2 border-gray-300 px-4 py-3 transition-all duration-200 focus:border-amber-500 focus:ring-4 focus:ring-amber-100 disabled:bg-gray-100 disabled:cursor-not-allowed">{{ $stockTransfer->notes }}</textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="sticky top-8 space-y-4 rounded-3xl bg-white p-6 shadow-xl">
                    ...
                    <a href="{{ route('inventory.stock-transfer.show', $stockTransfer->id) }}" x-ref="cancelButton"
...
function stockTransferForm() {
    return {
        formData: {
            ...
            movement_date: '{{ $stockTransfer->movement_date }}',
            notes: '{{ $stockTransfer->notes }}'
        },
        items: [],
        isDisabled: {{ $stockTransfer->status !== 'pending' ? 'true' : 'false' }},
