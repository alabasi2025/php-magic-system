...
<div class="inventory-page">
    <div class="container py-5">
        ...
        <div class="glass-header" data-aos="fade-down">
            <h1><i class="fas fa-box-open me-3"></i>أوامر التحويل المخزني</h1>
            <p class="subtitle">إدارة وتتبع جميع عمليات التحويل إلى المخازن</p>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card-3d" data-aos="fade-up" data-aos-delay="100">
                ...
                <div class="stat-label">إجمالي أوامر التحويل</div>
            </div>
        ...
        <div class="glass-table-container" data-aos="fade-up" data-aos-delay="400">
            <div class="table-header">
                <h2 class="table-title"><i class="fas fa-list me-2"></i>قائمة أوامر التحويل</h2>
                <a href="{{ route('inventory.stock-transfer.create') }}" class="btn-modern btn-primary-modern">
            ...
            <!-- Table -->
            @if(isset($stockTransfers) && $stockTransfers->count() > 0)
                    ...
                    <table class="modern-table">
                        ...
                        <tbody>
                            @foreach($stockTransfers as $stockTransfer)
                                <tr>
                                    <td><strong>{{ $stockTransfer->movement_number }}</strong></td>
                                    <td>{{ $stockTransfer->warehouse->name ?? '-' }}</td>
                                    <td>{{ $stockTransfer->movement_date }}</td>
                                    <td>
                                        <span class="status-badge status-{{ $stockTransfer->status }}">
                                            @if($stockTransfer->status == 'pending') قيد الانتظار
                                            @elseif($stockTransfer->status == 'approved') معتمد
                                            @else مرفوض
                                            @endif
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('inventory.stock-transfer.show', $stockTransfer->id) }}" class="action-btn action-btn-view">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($stockTransfer->status == 'pending')
                                            <a href="{{ route('inventory.stock-transfer.edit', $stockTransfer->id) }}" class="action-btn action-btn-edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <button class="action-btn action-btn-delete" onclick="confirmDelete({{ $stockTransfer->id }})">
...
<script>
    ...
    function confirmDelete(id) {
        if(confirm('هل أنت متأكد من حذف هذا الأمر؟')) {
            // Delete logic here
            console.log('Delete stock-transfer:', id);
        }
    }
</script>
@endsection
