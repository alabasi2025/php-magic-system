...
<div class="container-fluid px-4 py-3">
        ...
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-white-50 mb-0">رقم الأمر: {{ $stockIn->movement_number }}</p>
                </div>
                <div>
                    <a href="{{ route('inventory.stock-in.show', $stockIn->id) }}" class="btn btn-light btn-lg rounded-pill px-4">
    ...
    <form action="{{ route('inventory.stock-in.update', $stockIn->id) }}" method="POST" id="editStockInForm">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="warehouse_id" class="form-select form-select-lg" required {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-bold">تاريخ التوريد <span class="text-danger">*</span></label>
                        <input type="date" name="movement_date" class="form-control form-control-lg" 
                               value="{{ \Carbon\Carbon::parse($stockIn->movement_date)->format('Y-m-d') }}" 
                               required {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold">رقم الأمر</label>
                        <input type="text" class="form-control form-control-lg" value="{{ $stockIn->movement_number }}" disabled>
                    </div>
                    <div class="col-12">
                        <label class="form-label fw-bold">ملاحظات</label>
                        <textarea name="notes" class="form-control" rows="3" placeholder="أدخل أي ملاحظات إضافية..." {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>{{ $stockIn->notes }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover" id="itemsTable">
                        ...
                        <tbody>
                            ...
                            <tr class="item-row">
                                <td>
                                    <select name="items[{{ $index }}][item_id]" class="form-select item-select" required {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                                ...
                                <td>
                                    <input type="number" name="items[{{ $index }}][quantity]" class="form-control quantity-input" 
                                           value="{{ $item->quantity }}" step="0.001" min="0.001" placeholder="0" required {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                                </td>
                                <td>
                                    <input type="number" name="items[{{ $index }}][unit_cost]" class="form-control cost-input" 
                                           value="{{ $item->unit_cost }}" step="0.01" min="0" placeholder="0.00" required {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="text" name="items[{{ $index }}][batch_number]" class="form-control" 
                                           value="{{ $item->batch_number }}" placeholder="اختياري" {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                                </td>
                                <td>
                                    <input type="date" name="items[{{ $index }}][expiry_date]" class="form-control" 
                                           value="{{ $item->expiry_date }}" {{ $stockIn->status !== 'pending' ? 'disabled' : '' }}>
                                </td>
                                <td>
                                    @if($stockIn->status === 'pending')
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 15px;">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <a href="{{ route('inventory.stock-in.show', $stockIn->id) }}" class="btn btn-secondary btn-lg rounded-pill px-5">
...
<script>
let itemIndex = {{ $stockIn->items->count() }};
