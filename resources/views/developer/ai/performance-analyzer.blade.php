...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (code.length < 10) {
                alert('{{ $isRtl ? "يرجى إدخال كود يحتوي على 10 أحرف على الأقل" : "Please enter code with at least 10 characters" }}');
                return;
            }
        ...
        // دالة لتحديث النتائج في الصفحة
        function updateResults(data) {
            // تحديث السرعة
            if (data.speed) {
                document.getElementById('result-speed').textContent = data.speed;
            }
            // تحديث التعقيد
            if (data.complexity) {
                document.getElementById('result-complexity').textContent = data.complexity;
            }
            // تحديث Bottlenecks
            if (data.bottlenecks && data.bottlenecks.length > 0) {
                ...
                data.bottlenecks.forEach(item => {
                    ...
                    li.innerHTML = `<i class="fas fa-times-circle text-red-500 mt-1 mr-2 ml-2"></i><span>${item}</span>`;
            ...
            if (data.suggestions && data.suggestions.length > 0) {
                ...
                data.suggestions.forEach(item => {
                    const li = document.createElement('li');
                    li.className = 'flex items-start';
                    li.innerHTML = `<i class="fas fa-check-circle text-blue-500 mt-1 mr-2 ml-2"></i><span>${item}</span>`;
                    suggestionsList.appendChild(li);
                });
                document.getElementById('result-suggestions-count').textContent = data.suggestions.length;
            }
        }
