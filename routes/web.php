...
Route::get('/run-warehouse-accounts-seeder', function () {
    try {
        \Illuminate\Support\Facades\Artisan::call('db:seed', [
            '--class' => 'Database\\Seeders\\WarehouseAccountsSeeder',
            '--force' => true
        ]);
        $output = \Illuminate\Support\Facades\Artisan::output();
        return response()->json([
            'success' => true,
            'message' => 'Warehouse accounts seeder executed successfully',
            'output' => $output
        ]);
    } catch (\Exception $e) {
        return response()->json([
...
// Test route for creating purchase invoice
Route::get('/test-create-invoice', function () {
    try {
        ...
        $invoice = \App\Models\PurchaseInvoice::createPurchaseInvoice($data);
...
Route::get('/delete-all-purchase-invoices', function () {
    try {
        ...
        DB::table('purchase_invoice_items')->delete();
        
        // Then delete all invoices
        \App\Models\PurchaseInvoice::query()->delete();
