 ...
 * StockOutController
 * 
 * إدارة أوامر الصرف المخزني (إخراج البضائع)
 */
class StockOutController extends Controller
{
    public function index(Request $request)
    {
        ...
        $approvedOrders = StockMovement::where('movement_type', 'stock_out')->where('status', 'approved')->count();

        return view('inventory.stock-out.index', compact('stockOuts', 'warehouses', 'totalOrders', 'pendingOrders', 'approvedOrders'));
    }
    ...
    public function show(StockMovement $stockOut)
    {
        $stockOut->load(['warehouse', 'creator', 'approver', 'items.item']);
    ...
    public function approve(StockMovement $stockOut)
    {
        ...
        return back()->with('success', 'تم اعتماد أمر الصرف بنجاح');
    }
    ...
    private function generateMovementNumber(): string
    {
        $prefix = 'OUT';
        $date = now()->format('Ymd');
        $lastMovement = StockMovement::where('movement_type', 'stock_out')
